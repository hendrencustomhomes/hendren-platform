...
function rowSummary(item: TakeoffItem) {
  const kind = item.item_kind ? item.item_kind.toUpperCase() : 'ITEM'
  const parts = [kind, item.trade, item.cost_code, item.unit ? `${item.qty ?? '—'} ${item.unit}` : null].filter(Boolean)
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: '11px', fontWeight: '700' }}>{formatCurrency(assemblySubtotal)}</div>
        </button>
