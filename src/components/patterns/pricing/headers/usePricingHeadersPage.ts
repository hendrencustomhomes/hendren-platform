...
import { getPricingHeaderKindConfig } from '@/lib/pricing/headerKinds'
import type {
...
export type PricingHeaderStatusFilter = 'all' | 'active' | 'inactive'

export type UsePricingHeadersPageOptions = {
...
export function usePricingHeadersPage({
  kind,
  jobId = null,
  detailBasePath,
  permissionRowKey,
  accessDeniedMessage,
}: UsePricingHeadersPageOptions) {
  ...
  const [statusFilter, setStatusFilter] = useState<PricingHeaderStatusFilter>('all')
  ...
  async function loadPage() {
    ...
    try {
      const [headerRows, companyRows, tradeRows, costCodeRows] = await Promise.all([
        listPricingHeaders(supabase, { kind, job_id: jobId || undefined }),
      ...
      setError(e instanceof Error ? e.message : `Failed to load ${config.pluralLabel.toLowerCase()}.`)
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    async function initialize() {
      ...
      if (!accessResult.canView) {
        setLoading(false)
        setError(accessResult.error || accessDeniedMessage)
        return
      }
  ...
  async function handleCreate() {
    ...
    if (config.requiresJob && !jobId) {
      setError('Job is required for bids.')
      return
    }
    ...
    try {
      const header = await createPricingHeader(supabase, {
        kind,
        job_id: config.requiresJob ? jobId : null,
      ...
      await loadPage()
      router.push(`${detailBasePath}/${header.id}`)
    } catch (e) {
      setError(e instanceof Error ? e.message : `Failed to create ${config.singularLabel.toLowerCase()}.`)
    } finally {
      setSaving(false)
    }
  }
