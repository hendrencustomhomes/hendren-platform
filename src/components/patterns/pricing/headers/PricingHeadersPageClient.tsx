...
import { usePricingHeadersPage, PricingHeaderStatusFilter } from './usePricingHeadersPage'
import {
  inputStyle,
  labelStyle,
  pageStackStyle,
  ...
  cardStyle,
  pillStyle,
} from './pricingHeaderStyles'
import type { PricingHeaderKind } from '@/lib/pricing/types'

export default function PricingHeadersPageClient({
  kind,
  jobId,
  detailBasePath,
  permissionRowKey,
  ...
  const filters: PricingHeaderStatusFilter[] = ['all', 'active', 'inactive']
      ...
      <div style={pageStackStyle}>
        <div style={searchRowStyle}>
          <input
            ...
            placeholder={`Search ${state.config.pluralLabel.toLowerCase()}…`}
            style={inputStyle}
          />
        ...
        {state.showAdd && state.access?.canManage && (
          <div style={cardStyle()}>
            <div style={panelHeaderStyle}>New {state.config.singularLabel}</div>
            ...
            <div style={panelGridStyle}>
              ...
              <div style={{ ...fullWidthGridItemStyle, display: 'flex', justifyContent: 'flex-end' }}>
                <button onClick={state.handleCreate} disabled={state.saving} style={addButtonStyle}>
                  {state.saving ? 'Creating…' : `Create ${state.config.singularLabel}`}
                </button>
              </div>
            </div>
          </div>
        ...
        <div style={cardStyle()}>
          {state.loading ? (
            ...
            <div style={mutedMessageStyle}>No {state.config.pluralLabel.toLowerCase()}.</div>
          ) : (
            state.headers.map((h, i) => (
              <button
                key={h.id}
                onClick={() => router.push(`${detailBasePath}/${h.id}`)}
                style={{
                  display: 'block',
                  width: '100%',
                  textAlign: 'left',
        ...
        {!state.loading && (
          <div style={footerCountStyle}>
            {state.headers.length} {state.config.pluralLabel.toLowerCase()}
          </div>
        )}
      </div>
    </>
