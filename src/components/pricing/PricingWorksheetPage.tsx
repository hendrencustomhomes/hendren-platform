...
export default function PricingWorksheetPage({
    ...
    return (
      <>
        ...
        <div style={{ padding: '20px 16px' }}>
          ...
          <button type="button" onClick={() => router.push(backHref)} style={{ background: 'transparent', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: '8px', padding: '8px 12px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>Back</button>
        </div>
      </>
    )
  }
  ...
  if (!header) {
    return (
      <>
        <Nav title={navFallbackTitle} back={backHref} />
        <div style={{ padding: '20px 16px', fontSize: '13px', color: 'var(--text-muted)' }}>{missingLabel} not found.</div>
      </>
    )
  }
